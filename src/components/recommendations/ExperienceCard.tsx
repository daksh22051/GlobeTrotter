...
import { Star, MapPin, Compass, Bookmark, Check, Plus, Sparkles, Quote, Clock, Activity } from 'lucide-react';
...
export const ExperienceCard: React.FC<ExperienceCardProps> = ({
  ...
  const expInfo = item.experienceDetails;

  return (
    <div className="group flex flex-col bg-white rounded-3xl border border-[#EAE6DD] hover:border-[#6366F1]/40 shadow-xs hover:shadow-md transition-all duration-300 overflow-hidden">
      {/* Image Container */}
      <div className="relative h-48 sm:h-52 w-full overflow-hidden bg-[#202725]">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between z-10">
          <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-black/75 backdrop-blur-md border border-white/20 text-white text-xs font-black shadow-xs">
            ...
            <span className="text-[#A5B4FC]">{item.matchScore}%</span>
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white text-xs z-10">
          <div className="flex items-center gap-1 text-white/90 font-semibold truncate max-w-[60%]">
            <Compass className="w-3.5 h-3.5 text-[#A5B4FC] shrink-0" />
            <span className="truncate">{expInfo?.activityType || 'Signature Tour'}</span>
          </div>
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex items-start justify-between gap-2">
            <h3 className="text-base font-extrabold text-[#17201D] group-hover:text-[#6366F1] transition-colors leading-snug">
              {item.name}
            </h3>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            ...
            {expInfo?.fitnessLevel && (
              <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#4E5955] bg-[#F4F1EA] px-2.5 py-1 rounded-lg">
                <Activity className="w-3 h-3 text-[#20B8A6]" />
                <span>{expInfo.fitnessLevel}</span>
              </span>
            )}
          </div>
          ...
          {item.whyRecommended && (
            <div className="p-2.5 rounded-2xl bg-[#F5F5FE] border border-[#6366F1]/20 text-xs text-[#3E3887] flex items-start gap-2">
