...
export const HotelCard: React.FC<HotelCardProps> = ({
  ...
  const hotelInfo = item.hotelDetails;
  const nightPrice = hotelInfo?.pricePerNight || item.estimatedCost;

  return (
    <div className="group flex flex-col bg-white rounded-3xl border border-[#EAE6DD] hover:border-[#20B8A6]/40 shadow-xs hover:shadow-md transition-all duration-300 overflow-hidden">
      {/* Image Container */}
      <div className="relative h-48 sm:h-52 w-full overflow-hidden bg-[#202725]">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between z-10">
          ...
          <button
            ...
            title={isSaved ? 'Remove from bookmarks' : 'Save stay'}
          >
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white text-xs z-10">
          <div className="flex items-center gap-1 text-white/90 font-semibold truncate max-w-[60%]">
            <Building2 className="w-3.5 h-3.5 text-[#20B8A6] shrink-0" />
            <span className="truncate">{hotelInfo?.accommodationType || 'Boutique Stay'}</span>
          </div>
          <span className="px-2.5 py-0.5 rounded-md bg-white/20 backdrop-blur-md text-xs font-black">
            {formatCurrency(nightPrice, item.currency)}
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex items-start justify-between gap-2">
            <h3 className="text-base font-extrabold text-[#17201D] group-hover:text-[#20B8A6] transition-colors leading-snug">
              {item.name}
            </h3>
          ...
          {hotelInfo?.amenities && hotelInfo.amenities.length > 0 && (
            <div className="flex flex-wrap items-center gap-1.5 pt-1">
              {(hotelInfo.amenities || []).slice(0, 3).map((amenity, aIdx) => (
                <span
                  key={`${item.id}-amenity-${aIdx}-${amenity}`}
                  className="text-[10px] font-semibold text-[#179E8E] bg-[#EAF8F5] border border-[#20B8A6]/20 px-2 py-0.5 rounded-md"
                >
                  {amenity}
                </span>
              ))}
          ...
          {item.whyRecommended && (
            <div className="p-2.5 rounded-2xl bg-[#F2FBF9] border border-[#20B8A6]/20 text-xs text-[#136C61] flex items-start gap-2">
              <Quote className="w-3.5 h-3.5 text-[#20B8A6] shrink-0 mt-0.5" />
          ...
          <button
            ...
            className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-extrabold transition-all cursor-pointer shadow-2xs ${
              isAdded
                ...
                : 'bg-[#17201D] hover:bg-[#2C3B37] text-white shadow-xs'
            }`}
          >
            {isAdded ? (
              <>
