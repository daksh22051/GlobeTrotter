      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="print:hidden inline-flex items-center justify-center w-9 h-9 rounded-full bg-white border border-[#EAE6DD] text-[#556960] hover:text-[#17201D] hover:border-[#17201D] hover:bg-[#F9F7F1] transition-colors cursor-pointer shadow-2xs shrink-0"
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold tracking-wide uppercase bg-[#E8F8F5] text-[#20B8A6] border border-[#20B8A6]/20">
                ...
                {viewMode === 'calendar' ? 'Trip Calendar' : 'Trip Timeline'}
              </span>
            ...
            <h1 className="text-xl sm:text-2xl font-extrabold text-[#17201D] tracking-tight mt-0.5">
              {viewMode === 'calendar' ? 'Your trip calendar' : 'Your trip timeline'}
            </h1>
            <p className="text-xs sm:text-sm text-[#556960] hidden sm:block">
              {viewMode === 'calendar'
                ? 'Select a date to view that day’s scheduled activities.'
                : 'See every day, experience and moment of your journey at a glance.'}
            </p>
          </div>
        </div>

        {/* Right: View Switcher & Action CTAs */}
        <div className="print:hidden flex items-center gap-2 sm:gap-3 flex-wrap justify-between md:justify-end">
          {/* Undo Button */}
          {canUndo && onUndo && (
            <button
              type="button"
          ...
          {showViewSwitcher && <div className="inline-flex p-1 rounded-xl bg-[#F0ECE1] border border-[#EAE6DD]">
            <button
              type="button"
          ...
          {/* Primary Action: Optimize Schedule (Timeline only) */}
          {viewMode === 'timeline' && (
            <button
              type="button"
